$(document).ready(function() {
    ...
    $(document).on('change', '.purchase_quantity', function() {
        var is_accessories = $(this).attr('data-is-accessories');
        var variation_id = $(this).attr('data-variation-id');
        var row = $(this).closest('tr');
        var quantity = __read_number($(this), true);
        var product_id = __read_number(row.find('input.product_id'), true);
        ...
        // add imei
        if(is_accessories==0){
            if(typeof $(this).data('quantity-value') == 'undefined'){
                add_imei(quantity, product_id, row, variation_id)
            }else{
                var new_quantity = Number($(this).val());
                var index_value = $(this).data('index-value');
                var quantity_value = $(this).attr('data-quantity-value');
                $(this).attr('data-quantity-value',new_quantity);
                if(new_quantity>quantity_value){
                    var dif = Number(new_quantity)-Number(quantity_value);
                    add_imei_row(dif,quantity_value,index_value);
                }else{
                    for (let ind=Number(new_quantity); ind<Number(quantity_value); ind++){
                        var count = Number(ind)+1;
                        $('.quantity_value_'+index_value+'_'+count).remove();
                    }
                }
            }
        }
    ...
    // add imei
    function add_imei(id, product_id, row, variation_id){
        console.log(id, product_id, row, variation_id);
        $line = '';
        for (let index = 0; index < id; index++) {
            $line +=`<div><input type="text" name="imei1[`+product_id+`][`+variation_id+`][]"  placeholder="IMEI 1"></div>
            <div><input type="text" name="imei2[`+product_id+`][`+variation_id+`][]"  placeholder="IMEI 2"></div><br/>`
            
        }
        row.find('.imei').html($line);
    }

    function add_imei_row(number_to_add, start_after, product_serial){
        $line = '';
        for (let index = 0; index < number_to_add; index++) {
            var start_after = Number(start_after)+1;
            $line +=`<div class="quantity_value_`+product_serial+`_`+start_after+`"><input type="text" name="imei1[`+product_serial+`][`+start_after+`][]"  placeholder="IMEI 1"></div>
            <div class="quantity_value_`+product_serial+`_`+start_after+`"><input type="text" name="imei2[`+product_serial+`][`+start_after+`][]"  placeholder="IMEI 2"></div><br/>`
        }
        $('#imei_div_'+product_serial).append($line);
    }
