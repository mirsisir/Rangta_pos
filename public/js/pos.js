$(document).ready(function() {
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed) {
                        ...
                        var imei_info = ui.item.imei1;
                        if(ui.item.imei_number != ''){
                            imei_info = ui.item.imei_number;
                        }
                        pos_product_row(imei_info,ui.item.variation_id, purchase_line_id);
                    } else {
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed) {
                ...
                if(typeof item.imei1 != 'undefined' && item.imei1 != null && item.imei1 != ''){
                    string += '-' + item.imei1;
                }
                if(typeof item.imei2 != 'undefined' && item.imei2 != null && item.imei2 != ''){
                    string += '-' + item.imei2;
                }
    ...
    $('table#pos_table tbody').on('click', 'i.pos_remove_row', function() {
        $(this)
            .parents('tr')
            .remove();
        $('.edit_warranty_'+$(this).parent().parent().attr('data-row_index')).remove();
        pos_total_row();
    });
    ...
    $('select#product_category, select#product_brand, select#select_location_id, select#is_enabled_stock').on('change', function(e) {
        console.log($('select#is_enabled_stock').val());
    ...
    $(document).on('quickProductAdded', function(e) {
        ...
        if ($('input#location_id').val() == '') {
            ...
            pos_product_row(null,e.variation.id);
        }
    });
    ...
    $('button#weighing_scale_submit').click(function(){
        ...
        if($('#weighing_scale_barcode').val().length > 0){
            pos_product_row(null,null, null, $('#weighing_scale_barcode').val());
...
function pos_product_row(imei_id= null, variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
        ...
        $('#pos_table tbody')
            ...
            .each(function() {
                ...
                if (
                    ...
                    (row_imei_1 == imei_id || row_imei_2 == imei_id) &&
                    enable_sr_no !== '1' &&
                    !modifiers_exist &&
                    !is_added
                ) {
