		...
		<div class="row">
			...
			<div class="col-sm-12">
				<table class="table bg-gray" id="purchase_return_table">
					...
					<?php
							$imei_by_keys=[];
							foreach($imeis as $key=>$imei_each){
								$imei_by_keys[$imei_each->purchase_line_id][$key]=$imei_each;
							}
					?>
			            ...
			            <tr>
			              	...
			              	<td>
								...
								@isset($imei_by_keys[$purchase_line->id])
									@foreach($imei_by_keys[$purchase_line->id] as $itm)
										<input type="hidden" name="is_imei[]" value="{{$purchase_line->id}}">
										<input class="form-check-input" type="checkbox" name="imeis[{{$purchase_line->id}}][{{$itm->id}}]" value="1" id="flexCheckDefault">
										<label class="form-check-label" for="flexCheckDefault">{{'IMEI1 : '.$itm->imei1.' '.'IMEI2 : '.$itm->imei2}}</label><br>
									@endforeach
								@endisset
