    ...
    <section class="content">
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        <br>
                        <label>
                            {!! Form::checkbox('is_accessories', 1, !empty($duplicate_product) ? $duplicate_product->is_accessories : false, ['class' => 'input-icheck', 'id' => 'is_accessories']); !!} <strong>{{--@lang('product.is_accessories')--}} Has IMEI or Serial Number</strong>
                        </label>
                    </div>
                </div>
                <div class="clearfix"></div>
        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="row">
                ...
                <div class="col-sm-4" id="product_type" >
                    <div class="form-group">
                        {!! Form::label('type', __('product.product_type') . ':*') !!} @show_tooltip(__('tooltip.product_type'))<br>
                        {!! Form::select('type', $product_types, !empty($duplicate_product->type) ? $duplicate_product->type : null, ['class' => 'form-control select2',
                        'required', 'data-action' => !empty($duplicate_product) ? 'duplicate' : 'add', 'data-product_id' => !empty($duplicate_product) ? $duplicate_product->id : '0' ,'disabled']); !!}
                    </div>

                </div >

                <div class="col-sm-4" >
                    <div class="form-group" id="select_attribute" >
                        {!! Form::label('product_Variations', __('Select Product Variation') . ':') !!} <br>
                        {!! Form::select('product_Variations[]', $variation_templates, "90",
                            ['class' => 'form-control select2', 'multiple', 'id' => 'product_Variations' ,'disabled']); !!}
                    </div>
                </div>


                <div class="row col-sm-12 float-right"  id="select_variants" >
                    {{--        <p>Add Variation</p>--}}
                    <div class="col-sm-9">
                        <div class="form-group" id="variants_value">
                ...
                <div class="text-center">
                    <div class="btn-group">
                        ...
                        @can('product.opening_stock')
                            ...
                            type="submit" value="submit_n_add_opening_stock" class="btn bg-purple submit_product_form">@lang('lang_v1.save_n_add_opening_stock')</button>
                        @endcan
...
@section('javascript')
    ...
    <script type="text/javascript">
        var all_selected_variant = [] ;

        $(document).ready(function(){
            // document.getElementById("select2-type-container").disabled = false;

            $("#type").removeAttr('disabled');
            $("#product_Variations").removeAttr('disabled');
        ...
        $('#product_Variations').change(function() {
            ...
            let selected_variant = $('#product_Variations').val();
            let update_variant = selected_variant[selected_variant.length - 1];
            let update= true;

            if ($('#variant_value_id_'+update_variant).length) {
                update= false;
            }

            if (window.all_selected_variant !==[]){
                let difference = window.all_selected_variant.filter(x => !selected_variant.includes(x));
                console.log("difference" ,difference)
                $( "#variant_value_id_"+difference ).remove();
                $( "#variant_value_id_"+difference ).remove();
                $( "#variant_value_id_"+difference ).remove();
            ...
            window.all_selected_variant = $('#product_Variations').val()

            window.all_selected_variant.forEach((item)=>{
                if ($('#variant_value_id_'+item).length == 0) {
                    update_variant = item;
                    update= true;
                }
            })

            console.log('update_variant', update_variant)

            if (update == true){
                $.ajax({
                    method: 'POST',
                    url: '/products/variant_value',
                    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    dataType: 'html',
                    data: {   update_variant: update_variant },
                    success: function(result) {
                        if (result) {
                            $('#variants_value').append(result);
            ...
            update_product_variant_list()
        });

        function update_product_variant_list(){
            let selected_variant = window.all_selected_variant;
            let selected_variant_value = {};
            selected_variant.forEach((item, index)=>{
                let v_value = $('#variant_'+item).val()
                selected_variant_value[index] = v_value;
            })

            show_product_variant_form(selected_variant_value);

        }

        function show_product_variant_form(selected_variant){

            $.ajax({
                method: 'POST',
                url: '/products/product_form_variant',
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                dataType: 'html',
                data: {   selected_variant_value: selected_variant },
                success: function(result) {
                    if (result) {
