...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
			...
			if(!empty($product->imei_id)){ $product_name .= ' IMEI-1: '.$product->imei1.' IMEI-2: '.$product->imei2 ;}
		@endphp
	...
	<td>
		...
		@if(!empty($product->imei_id))
			<input type="hidden" value="{{$product->imei_id}}"
				   name="products[{{$row_count}}][imei_id]" class="row_imei_id">
			<input type="hidden" value="{{$product->imei1}}"
				   name="products[{{$row_count}}][imei_1_number]" class="row_imei_1">
			<input type="hidden" value="{{$product->imei2}}"
				   name="products[{{$row_count}}][imei_2_number]" class="row_imei_2">
		@endif
	...
	@else
		@if(!empty($warranties))
			{!! Form::select("products[$row_count][warranty_id]", $warranties, $warranty_id, ['placeholder' => __('messages.please_select'), 'class' => "form-control edit_warranty_$row_count"]); !!}
		@endif
