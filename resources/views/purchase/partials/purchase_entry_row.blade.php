@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            <input type="text" 
                name="purchases[{{$row_count}}][quantity]" data-is-accessories="{{$product->is_accessories}}" data-variation-id="{{$variation->id}}"
        ...
        @if(!$product->is_accessories)
            <td>
                <div class="imei">
                    <div><input type="text" name="imei1[{{$product->id}}][{{$variation->id}}][]"  placeholder="IMEI 1"></div>
                    <div><input type="text" name="imei2[{{$product->id}}][{{$variation->id}}][]"  placeholder="IMEI 2"></div>
                </div>
            </td>
        @else
            <td>
