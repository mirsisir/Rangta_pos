...
<div class="table-responsive">
    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        <?php $parent_loop_index = $loop->index;?>
        <tr @if(!empty($purchase_line->purchase_order_line) && !empty($common_settings['enable_purchase_order'])) data-purchase_order_id="{{$purchase_line->purchase_order_line->transaction_id}}" @endif>
            ...
            <td>
                ...
                value="{{@format_quantity($purchase_line->quantity)}}" data-is-accessories="{{$purchase_line->product->is_accessories}}" data-index-value="{{$loop->index}}" data-quantity-value="{{$purchase_line->quantity}}"
            ...
            @if(!$purchase_line->product->is_accessories)
                <td id="imei_div_{{$loop->index}}">
                    <?php
                    $x=1;
                    ?>
                    @foreach($detect_imeis[$purchase_line->id][$purchase_line->variation_id] as $imei_numbers)
                        <div class="quantity_value_{{$parent_loop_index}}_{{$x}}"><input type="text" name="imei1[{{$parent_loop_index}}][{{$x}}][]" value="{{$imei_numbers->imei1}}" placeholder="IMEI 1"></div>
                        <div class="quantity_value_{{$parent_loop_index}}_{{$x}}"><input type="text" name="imei2[{{$parent_loop_index}}][{{$x}}][]" value="{{$imei_numbers->imei2}}" placeholder="IMEI 2"></div>
                        <br/>
                        <?php $x++; ?>
                    @endforeach
                </td>
