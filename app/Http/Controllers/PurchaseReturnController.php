...
class PurchaseReturnController extends Controller
{
    ...
    public function add($id)
    {
        ...
        $imeis = Imei::whereIn('purchase_line_id',$purchase_lines_ids)->get();
        return view('purchase_return.add')
                    ->with(compact('purchase','imeis'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $is_imei = $request->input('is_imei') ?? [];
            foreach ($return_quantities as $pur_id=>$item_R){
                if($item_R>0 && in_array($pur_id,$is_imei)){
                    if(isset($imeis[$pur_id]) && count($imeis[$pur_id])==$item_R){

                    }else{
                        $output = ['success' => 0,
                            'msg' => __('Please select IMEI')
                        ];
            ...
            $imeis_ids=[];
            foreach($imeis as $test1){
                foreach($test1 as $key=>$value_yes){
                    $imeis_ids[]=$key;
                }
            }
            if($imeis_ids){
                Imei::whereIn('id',$imeis_ids)->delete();
            }
            $business_id = request()->session()->get('user.business_id');
