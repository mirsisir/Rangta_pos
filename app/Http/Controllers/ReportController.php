...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($for == 'view_product' && !empty(request()->input('product_id'))) {
                ...
                $product_imeis = Imei::where('product_id', request()->input('product_id'))->where('status', 0)->orderBy('id', 'Desc')->get();
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $imei_id = $request->get('imei_id_value', null);
            ...
            if($imei_id){
                $query->join('imeis', 'imeis.purchase_line_id', '=', 'purchase_lines.id');
            }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if($imei_id){
                $query->where('transaction_sell_lines.imei_id', $imei_id);
            }
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            if (!empty($start_date) && !empty($end_date)) {
