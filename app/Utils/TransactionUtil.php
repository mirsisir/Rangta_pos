...
class TransactionUtil extends Util
{
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    if(isset($product['imei_id']) && !empty($product['imei_id'])){
                                        $new_arr['imei_id']=$product['imei_id'];
                                    }
                                    $modifiers_formatted[] = new TransactionSellLine($new_arr);
                                }
                            }
                        }
                    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell') {
            ...
            foreach ($lines as $key => $value) {
                ...
                if(isset($value->imei_id) && !empty($value->imei_id)){
                    $imeis = Imei::where('id','=',$value->imei_id)->first();
                    $value->imei1 = $imeis->imei1;
                    $value->imei2 = $imeis->imei2;
                }
            }
    ...
    public function getPurchaseProducts($business_id, $transaction_id)
    {
        $products = Transaction::join('purchase_lines as pl', 'transactions.id', '=', 'pl.transaction_id')
                            ...
                            ->select('p.id as product_id', 'p.name as product_name', 'p.product_custom_field1 as product_custom_field1', 'v.id as variation_id', 'v.name as variation_name', 'pl.quantity as quantity', 'pl.exp_date', 'pl.lot_number')
                            ->get();
        return $products;
    }
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ...
                // Comments By : Rajan Bhatta, Date: 09-03-2022
    ...
    * @return object
    */
    public function getListSells($business_id, $sale_type = 'sell',$imei_id=null)
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                // ->leftJoin('transaction_payments as tp', 'transactions.id', '=', 'tp.transaction_id')
                ->leftJoin('transaction_sell_lines as tsl', function($join) use ($imei_id) {
                    $join->on('transactions.id', '=', 'tsl.transaction_id')
                        ->whereNull('tsl.parent_sell_line_id');
                    if($imei_id){
                        $join->where('tsl.imei_id','=',$imei_id);
                    }
                ...
                if($imei_id){
                    $sells->where('tsl.imei_id','=',$imei_id);
                }
                $sells->select(
                    'transactions.id',
                    'transactions.transaction_date',
