...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true,$imei_id=null)
    {
        if ($uf_data) {
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            ...
            if($imei_id){
                Imei::where('id','=',$imei_id)->update(['status'=>1]);
            }
     ...
     * @return boolean
     */
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0,$imei_id=null)
    {
        $qty_difference = $new_quantity - $old_quantity;
     ...
     * @return array
     */
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true, $imei_id=null)
    {
        ...
        $query = Variation::join('products AS p', 'variations.product_id', '=', 'p.id')
                ...
                ->leftJoin('imeis', function($join)
                {
                    $join->on('imeis.product_id', '=', 'variations.product_id');
                    $join->on('imeis.variation_id','=', 'variations.id');
                    $join->where('imeis.status','=', 0);
                })
       ...
       /* if ($check_qty) {
        ...
        if ($imei_id) {
            $query->where(function ($query) use ($imei_id) {
                $query->where('imeis.imei1', '=', $imei_id);
                $query->orWhere('imeis.imei2', '=', $imei_id);
            });
        }

        // Location Comments by : Rajan Bhatta, Date: 09/03/2022
      ...
      /*  if (!empty($location_id) && $check_qty) {
        ...
        //->firstOrFail()
            ->groupBy('vld.product_id')->firstOrFail();
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            foreach ($input['products'] as $product) {
                if (!empty($product['transaction_sell_lines_id'])) {
                    $this->updateProductQuantity($input['location_id'], $product['product_id'], $product['variation_id'], $product['quantity'], 0, null, false,$product['imei_id']);
            ...
            foreach ($input['products'] as $product) {
                ...
                $this->decreaseProductQuantity(
                    ...
                    $input['location_id'],
                    $uf_quantity,0,$product['imei_id'] ?? null
                );
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like'){
        $xv='';
        $search_imei_result = Imei::where('imei1','=',$search_term)->orWhere('imei2','=',$search_term)->first();
        if($search_imei_result){
            $xv = $search_term;
        }
        ...
        $query = Product::join('variations', 'products.id', '=', 'variations.product_id')
               ...
               ->leftjoin(
                    ...
                    function ($join) use ($location_id) {
                        ...
                        //Include Location
                        // Comment By : Rajan Bhatta, Date: 09-03-2022
            ...
            if($search_type == 'like'){
                $query->where(function ($query) use ($search_term, $search_fields) {
                    ...
                    if (in_array('imei', $search_fields)) {
                        $query->orWhere('imei1', '=', $search_term);
                        $query->orWhere('imei2', '=', $search_term);
                    }
        ...
        $query = $query->addSelect(DB::raw("'$xv' as 'imei_number'"));
        ...
        $query->groupBy('variations.id','imeis.id');
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        if (!empty($imei_number)) {
            $query = $query->leftJoin('imeis', 'p.id', '=', 'imeis.product_id');
        }
        ...
        $products = $query->select(
            ...
            DB::raw("(SELECT SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ) FROM transactions 
                  JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                  WHERE transactions.status='final' AND transactions.type='sell_transfer' AND (TSL.variation_id=variations.id)) as total_transfered"),
            ...
            DB::raw('group_concat(l.name) as location_name'),
            'l.id as location_id',
